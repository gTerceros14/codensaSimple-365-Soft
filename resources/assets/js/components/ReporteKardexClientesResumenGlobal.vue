...
methods: {
    ...
    generarReporteVista(page, condicion) {
        ...
        if (me.clienteId != false) {
            ...
            if (condicion == 1) {
                console.log('entrando con condicion 1');
                axios.get(url).then(function (response) {
                    var respuesta = response.data;
                    me.arrayDetallesReporte = respuesta.detalles;

                    console.log('DETALLES: ', me.arrayDetallesReporte);
                })
                .catch(function (error) {
                    console.log(error);
                });
    ...
    exportarExcel() {
        if (this.arrayDetallesReporte.length != 0) {
            ...
            const headers = ['Cliente','Comprobante', 'Tipo', 'Fecha Venta', 'Articulo', 'Fecha Vencimiento', 'Unidades Vendidas',
            ...
            for (let i = 0; i < this.arrayDetallesReporte.length; i++) {
                const { nombre_cliente, num_comprobante, tipo_comprobante, fecha_hora, nombre, 
                    fecha_vencimiento, unidades_vendidas, precio_por_unidad, descuento, precio_total_venta, total_con_descuento,
    ...
    exportarPDF() {
        if (this.arrayDetallesReporte.length != 0) {
            ...
            const columns = ['Cliente','Comprobante', 'Tipo', 'Fecha Venta', 'Articulo', 'Fecha Vencimiento', 'U.V.',
            ...
            const rows = this.arrayDetallesReporte.map(datos => [datos.nombre_cliente ,datos.num_comprobante, datos.tipo_comprobante, datos.fecha_hora, datos.nombre, 
                datos.fecha_vencimiento, datos.unidades_vendidas, datos.precio_por_unidad, datos.descuento, datos.precio_total_venta, datos.total_con_descuento,
